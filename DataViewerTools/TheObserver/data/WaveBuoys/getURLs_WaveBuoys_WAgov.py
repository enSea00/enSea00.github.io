...
def csv_to_json_with_column_mapping(csv_file_path, json_file_path, column_mapping):
    ...
    with open(csv_file_path, mode='r', encoding='utf-8') as csv_file:
        ...
        for row in csv_reader:
            if row['Live_Wave'] == '': # skip jistorical buoys which have no "live" url
                continue
            filtered_row = {'DataType': 'Wave Buoy'}
            notes = f'{row['Location_ID']}, depth = {row['Depth']}'
