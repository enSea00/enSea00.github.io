...
// Modify the plotData function to plot inside the popup
function plotData(groupedData, site) {
    const tracesHsig = [];
    const tracesHmax = [];
    const tracesTp = [];
    ...
    // Trigger Plotly to render inside the popup
    Plotly.newPlot('plot', traces, layout);

    // Open the popup after plotting the graph
    openPopup();
}

// Map selection function (modification)
function selectSite(selectedSite) {
    Object.keys(markers).forEach(site => {
        markers[site].setStyle({
            fillColor: defaultColor,
    ...
    plotData(groupedData, selectedSite); // Plot data inside the popup
}
